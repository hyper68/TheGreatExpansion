...
public class CrushedRecipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;

	public CrushedRecipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems) {
		this.output = output;
	...
	public static class Type implements RecipeType<CrushedRecipe> {
		private Type() {
		}

		public static final RecipeType<CrushedRecipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<CrushedRecipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<CrushedRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems)).apply(builder, CrushedRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, CrushedRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<CrushedRecipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, CrushedRecipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static CrushedRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new CrushedRecipe(outputs, inputs);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, CrushedRecipe recipe) {
